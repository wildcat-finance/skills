...
**What changed.** Complete replacement Exit: `scripts/render_harness_roster.py --check` runs inside the repository suite through a declared scope, so a drifted surface reddens an ordinary run. The guide sentence claiming the two web routes are covered by Atlas launcher tests is verified against the Atlas repository at `ce866e3d7e8b489fcb8b70c608f7af72d9b7a673` and kept, because that tree holds six test files and `tests/rendered-html.test.mjs` tests both routes: a loop over `chatgpt` and `claude` asserts `/go/{provider}` returns 307 with `cache-control: no-store`, that the redirect origin is `https://chatgpt.com` or `https://claude.ai`, and that the destination's `q` parameter equals a prompt the same run read from `/api/job?all=true`. Proved by `python3 scripts/run_checks.py --full` at exit zero on the committed tree, with a deliberate one-character edit to a generated surface shown to redden it and then reverted. Complete replacement Tests: `DriftTests`: the check mode is reachable from the repository suite, fails on a changed surface, fails on a missing manifest, and fails when the manifest's recorded run does not match the surfaces being checked. `AtlasClaimTests`: the guide's claim that the two routes are covered agrees with what the Atlas repository holds at the pinned commit, the limiting sentence after it survives so the bootstrap is never read as evidence that a browser chat can edit a local checkout, sign commits, restore a checkpoint or publish through an account, and neither surface claims a checked one-click launcher for any local harness. Expect around six new cases. Step audit runner contract is test command `python3 tests/run_tests.py --elenchus-report {report}`, report format `elenchus.unittest.v1`, report file `.elenchus/fiat-856-step-4.json`.

**Why.** The Exit rested on a claim about another repository that nobody checked before writing it into a step's exit condition. It stated that `ce866e3d` holds six test files and no route test, and ordered the guide sentence replaced by that. The commit holds six test files and a route test: `tests/rendered-html.test.mjs` lines 98 to 114 parameterise over `chatgpt` and `claude` and assert the 307, the `no-store` header, the destination origin and that the redirect carries a prompt from the API's own job pool. So the guide sentence at `docs/how-to-help-shoggoth.md:117` is true as written, and building the step to the receipted Exit would have replaced a true sentence with a false one and pinned the falsehood in `AtlasClaimTests`. Every one of step 3's seven audit rounds recorded the same unknown, that the Atlas repository was not fetched in any round; it was closed here with one API read. The drift half of the Exit is unchanged, and the step's Goal, Entry, Files and Disciplines are unchanged.
...
**Why.** This reissues the fields the amendment of the same date already replaced, against the current study. That amendment recorded study digest `e858c4c94905d64e83256d1c07fa235b720c467701cd6aa7db6c49e44b71fee3`, and the study amendment of this date, which superseded the false `atlas-claim` register row, moved the digest to `0ed22b1513a756ad4921c186e7537dec08d4a22c601fecdff3684c55b8f02d7c`. The controller drops a runbook amendment whose recorded study digest is not current, so the reissue is required to keep step 4's effective `Exit` and `Tests` from reverting to a baseline that orders a true sentence replaced with a false one. Step 4's work is unchanged and was built and audited against these fields. This is the run's fifth such reissue; the treadmill is a controller defect carried to the run's carryover block rather than answered here.
