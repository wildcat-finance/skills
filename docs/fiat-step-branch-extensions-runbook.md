...
**What changed.** Complete replacement Files: Change
`plugins/hexaemeron/skills/fiat/scripts/hexctl.py`,
`plugins/hexaemeron/tests/test_step_branch_extensions.py`, including only the
focused direct-run report mode needed to accept one `--elenchus-report` path,
run this module's ten tests and delegate secure target binding, the exact
`elenchus.unittest.v1` payload and exclusive report creation to the existing
`run_tests.py` reporter functions; change
`plugins/hexaemeron/tests/test_push_receipt_identity.py`, only the
`RewrittenStackRefusal` fixture, description, and assertions in
`plugins/hexaemeron/tests/test_hexctl.py` needed to drive native ancestry
status 1 and require the non-ancestor diagnostic without the unobserved GitHub
...
signed and the tree is clean, generate `.dead-code/baseline.json` with
`python3 scripts/dead_code.py baseline --write` and publish it as the only path
in a separate signed descendant commit. No generic `run_tests.py` interface,
unrelated test, Homologia path or other product path is in scope.

Complete replacement Tests: Preserve the raw-equality failure against Step
2's entry and the passing implementation result. Run every focused case and
complete suite named in Step 2's `Prove the exit with` block, plus every test
added by the earlier amendments. Every focused behavioural, legacy, version,
generated-runtime, coverage, lint, prose, Horos, dead-code and diff command
must exit zero. The corrected source-bound audit-fix runner is
`python3 plugins/hexaemeron/tests/test_step_branch_extensions.py --elenchus-report {report}`;
the CLI report format is `unittest-json-v1`, the report file is
`.elenchus/fiat-923-step-2.json`, and the expected JSON schema and counters are
exactly `elenchus.unittest.v1`, `complete: true`, `testsRun: 10`,
`failures: 0`, `errors: 0`, `skipped: 0`, `expectedFailures: 0`, and
`unexpectedSuccesses: 0`. The `{report}` placeholder occurs exactly once.
Warden must return the exact Elenchus verdict `passed`; a missing, stale,
incomplete, malformed, redirected or differently counted report is
`inconclusive` and must not be translated into a guard or pass.

The complete unfiltered `python3 plugins/hexaemeron/tests/run_tests.py` census
must separately execute its whole manifest. On this Darwin checkout it may
exit 3 only with exactly 1,834 executed tests, two failure events, one error
event and zero skips, all three matching the entry evidence below; any
additional or changed failure, error, skip, missing test or scheduler fault
refuses completion:
...
**Why.** Warden ran the previously receipted command twice and each invocation
exited 2 before discovery: `run_tests.py` defines its sole positional value as
the legacy alternative report path, so the appended test filename became a
second report path beside `--elenchus-report`. That runner exposes no public
positional test-selection interface and created no report. The corrected mode
lives in the already-scoped focused test file, uses the existing reporter as
the one schema and path authority, and leaves the parallel runner's public
contract unchanged.
