...
and run records under `docs/promise-machine/obligation-gates/`, ADR-069, and
...
evidence-substitution failures start red and remain guards. hypomnema: ADR-069
...
**What changed.** Complete replacement Disciplines: phylax: composition consumes two bounded local result surfaces and must preserve both boundaries without running either producer or consumer. ephoros: every stop names relation, producer, consumer, lost field, blocked transition, and recovery. metron: none, no performance claim. elenchus: each removed relation or field is an observed guard. hypomnema: ADR-069 owns the obligation and composition contract, so no second decision record is needed.
**Why.** The current default branch now owns ADR-062 through ADR-065, so the root uniqueness check rejects the previously receipted ADR-062 identifier. The study amendment records ADR-069 as the next collision-free standing identifier. This replacement changes only Step 4's decision reference; its entry, exit, files, tests, evidence boundaries, and seven-relation design remain unchanged.
