...
**What changed.** Scenario condition vectors must be independent of host-route ordering. For each selected canonical skill, derive realizable vectors using the existing semantic priority: prefer the condition target's canonical owner, then the shortest conditional dependency chain. Emit every vector on every host base tied at that semantic priority; use repository, isolated Agent Skills and standalone route order only for deterministic record ordering, never to select one route. The repository and isolated Agent Skills bases each retain the credential-backed contributor vector; standalone does not. The corrected graph has 19 roots, 237 host edges, 656 scenario roots, 277 scenario edges and six reference-only records. Across 31 selected skills, each route has the same 198 non-credential vectors; repository and isolated Agent Skills each add 31 credential vectors, yielding route totals 229, 229 and 198. Ariadne is 7/7/6, Kronos is 28/28/27 and Synkrisis is 5/5/4 across repository, isolated Agent Skills and standalone. Every prior branch-exclusivity, mandatory-execution and Kronos 25-ledger ranking rule remains.

The 188-file corpus, byte partition, development/holdout membership and byte denominators do not change. Scenario reachability grows, so the manifest, graph, manifest-bound partition and cohort records, artifact inventory, reconciliation and seal commitment must be regenerated together. The sealed membership, membership digest, case envelope digest and `opened: false` state remain unchanged. Steps 2 through 5 inherit the corrected scenario and artifact identities before any candidate, case or model execution.

**Why.** The builder currently chooses `min(candidates)` after including host route in the priority. When repository, isolated Agent Skills and standalone are semantically equivalent, repository wins and the other realizable invocations disappear. The validator then derives each edge scope only from already-declared roots, so the graph remains self-consistent while incomplete. The current graph consequently assigns Ariadne `capture-release`, Synkrisis `diagnose` and each noncanonical Kronos target to only one repository scenario. That understates scenario-reachable and later complete-prompt denominators despite the study's requirement that each declared invocation bind its own host route and firing condition vector.
