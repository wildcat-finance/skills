...
The published study/design snapshot keeps report copies under `docs/protasis-success-criteria/reports/`; the active controller consumes `.hexaemeron/design-evidence.json` and its local reports. After earning a conformance report, copy its unchanged bytes to the matching published report home before checking the published snapshot at the same transition. Never rewrite the controller's historical receipts to make a documentation copy current.
...
**What changed.** The operating boundaries already applied by the source contracts and complete runbook are explicit here. Always preserve source-bound evidence, run the root and Hex suites before a commit, use the normal staged commit gate, keep valid signatures, pass the selected repository checks and prose checks, and preserve native checkpoints. Ask first before a dependency, storage/public ABI change, CI change, trust-boundary expansion or released-digest rewrite outside the accepted #1273 design. The declared criterion schema, execution operation, result custody and compatibility changes are already authorised. Never fabricate an execution or verdict, edit controller state or ledger by hand, change the fixed external controller, erase a failing test or frozen record, expose credentials, bypass merge gates or force-push over another contributor.
**Why.** The study distributed these rules across its constraints and discipline sections; Protasis also requires an explicit always, ask-first and never statement. This clarification preserves the selected design, all eight criteria, every declared limit, the no-known inventory and the implementation order.
**Steps touched.** Steps 1, 2, 3, 4 and 5 use the same operating boundaries already stated in the complete runbook.
...
**What changed.** After the prerequisite repair is merged through its normal checks, replace the outer run's external controller once with a separate source checkout of signed commit `c572009b7922b4b7b9400893c5adc6d4df641a72`, Fiat `6.64.1`, controller SHA-256 `9ca0576cf335b8df7f124b6235f87df4b60e7c290823229becd57227ea1e7359`. Preserve the original source checkout at `eaab110104d186bdd0b1d362dd583a2e72ecc21a` and its controller bytes unchanged. This is the sole exception to the fixed-controller rule; the newly published source then stays fixed for all remaining steps. Use the existing runbook amendment recovery to capture its command-adapter digest. Preserve the immutable init currency observation and record the adoption separately. The published source also carries current main's refusal of trusted GitHub web-flow signatures and its fixed search path for execution tools. These remain part of that fixed source; the declaration commands stay unchanged. Step 1 also receives the published path-escape fixture repair in `plugins/hexaemeron/tests/test_imprimatur_family_evidence.py`, preserving its actual symlink refusal and occupied-sibling check. Publish the amended study and runbook, retained evidence and fresh source-bound design-home proof. Before the separate new-controller demonstration, bring the published controller repair into the target through a bounded admitted amendment or ordinary signed integration sync, then rerun the affected conformance evidence. Changing the external driver alone does not update the target.
**Why.** Native Git reproductions show that the original guard rejects the clean committed audit pair required by its audit workflow. No live Step 2 transition was attempted. The repair binds committed audit bytes to the receipt, native HEAD and exact index while preserving the existing untracked route and physical checks. A full suite also reproduced a collision at the shared `escaped-schemas` temporary path; the repaired test owns its parent and retains the refusal. A read-only compatibility probe left the current state, ledger and original controller unchanged. Its only gate-record difference was the adapter digest; every declared command matched. The user authorised the necessary repair with "Approval for anything you need." Publication, signatures, normal commit and merge gates, receipts and append-only history remain required. This outer run still does not demonstrate the new success-criteria feature.
**Steps touched.** Steps 1, 2, 3, 4 and 5 use the one published external controller after adoption. Step 1 republishes the amended source and fixture evidence; later target adoption remains separately bounded before the new-controller demonstration.
**Still holding.** Step 1: entry holds; exit holds. Step 2: entry holds; exit holds. Step 3: entry holds; exit holds. Step 4: entry holds; exit holds. Step 5: entry holds; exit holds.
