...
**What changed.** This document refers to `.hexaemeron/` at lines 5, 256, 411, 412 and 467. That directory is self-ignored controller evidence and never reaches the repository, so for a reader holding only the repository each of those references resolves to nothing. In the shipped tree the same artefacts sit beside this study: the design record at `docs/commit-gate/design-evidence.json`, the forty reports at `docs/commit-gate/reports/`, and the runbook at `docs/commit-gate/runbook.md`. Read every `.hexaemeron/<name>` above as `docs/commit-gate/<name>` when reading the shipped copy. The evaluator at `.hexaemeron/design/evaluate_commit_gate.py` is not shipped, so the reports it wrote are the only surviving evidence of the measurements this study quotes, and the budget command at line 467 cannot be re-run from the repository alone.

**Why.** Audit finding S1-R1-01, medium, on step 1 round 1. Lines 411 and 412 send the reader to `.hexaemeron/reports/` for the forty reports that ship at `docs/commit-gate/reports/`, which is the citation a reader is most likely to follow. Neither check that would normally catch a dead pointer sees it: Hypomnema's H001 reads Markdown links rather than code spans, and `tests/test_contributors.py` names this class but its specification does not cover `docs/commit-gate/`. A study is append-only once receipted, so the five references cannot be rewritten where they stand; this block records the mapping instead, and the shipped copy is refreshed to carry it at step 4.
...
**What changed.** The amendment above names five `.hexaemeron/` references in this study; there are seven, at lines 5, 256, 329, 411, 412, 465 and 467. Lines 329 and 465 open their code span on the line before, which is why the shorter count was taken. The mapping recorded above applies to all seven. The budget command that amendment calls "the command at line 467" is at lines 465 and 466; line 467 is its report path. The shipped runbook carries five `.hexaemeron/` references of its own. Two of them, at its lines 3 and 83, name this study and the run's artefacts and take the same mapping. The other three, at its lines 43, 178 and 189, name the exact path the controller requires step 3 to write its conformance report to, so they are working paths for the run rather than citations to a shipped file, and they are correct as they stand.

**Why.** Audit findings S1-R2-01, medium, and S1-R2-04, low, on step 1 round 2. The round 2 review recounted the references and found the previous amendment's enumeration short by two, so the remedy it recorded did not reach every citation it was written to cover. Recording a correction that is itself incomplete is worse than recording none, because the shorter count reads as exhaustive. This amendment also changes this study's digest, which unbinds every runbook amendment recorded before it; both are re-issued immediately after it for that reason.
...
**What changed.** Section 10's budget clause is wrong as written and is corrected here. It reads that the gate adds at most 200 milliseconds to a commit whose tree is already recorded green, with no qualifier. Step 2 round 2 measured first commits at 219.61, 253.05 and 322.49 milliseconds across three independent fresh repositories, each given five ungated warm-up commits before the gate was activated. Those are commits whose tree is already recorded green, so they fall inside the clause's own terms and breach it. The budget is therefore scoped to steady state: the gate adds at most 200 milliseconds to a commit whose tree is already recorded green, once the hook file has been executed at least once on that machine. Beside it stands the cost the clause did not account for: the first commit after the hook file is written pays the operating system's first-execution assessment as well, measured at 219.61 to 322.49 milliseconds, and a hook whose entire body is a bare success pays 182.79 of it. That cost is paid again whenever the hook file is rewritten, so a contributor meets it after a clone and after any pull that changes the gate. Steady state measured 31.91 milliseconds for a do-nothing hook and 32.49 for the shipped gate on the same machine, so no gate design avoids the first-execution cost and the shipped gate adds nothing measurable to it.

**Why.** Audit finding S2-R2-02, medium, open and controller-side, on step 2 round 2. The round was asked to rule on whether the cold first commit breached the stated budget or the harness was merely cold, and not to soften the answer to protect the step. It ruled breach, and separated the cause rather than arguing it: with the ungated arm timed first the outlier still follows the gate, at 52.45 milliseconds ungated against 682.89 gated; a cold shell exec accounts for about 11 milliseconds, not 200; and the cost recurs per hook file, returning at 275.40 and 429.28 milliseconds when the hook was rewritten mid-run with neighbours at 73 to 92. Leaving the original clause would leave a claim in a shipped document that its own numbers refute. One honesty marker carried from the round: the measurements are recorded, while the attribution to the operating system's first-execution assessment is inferred from the per-file recurrence signature on one loaded macOS arm64 machine, with no instrumentation of the responsible service.
...
**What changed.** Four corrections to the budget amendment recorded immediately above. First, its steady-state pair is misattributed and its conclusion from that pair is withdrawn. The 31.91 milliseconds was the step 2 original hook's steady state, not a hook that does nothing; round 2 measured a do-nothing hook at 10.55 milliseconds, and round 3 re-timed both over three fresh repositories at 6.49, 2.13 and 7.03 milliseconds for a do-nothing hook against 18.97, 23.20 and 20.57 for the shipped gate. What the 31.91 and 32.49 pair supports is that round 1's fix costs nothing over the gate it replaced. It does not support the claim that the shipped gate adds nothing measurable, and that claim is withdrawn: the gate's own logic costs roughly 14 milliseconds today and roughly 21 in round 2 over a hook that does nothing. The budget verdict is untouched, because 20.57 milliseconds sits far inside 200. Second, the words "on that machine" are struck from the scoping clause. The first-execution cost is paid per file instance, not per machine and not per content: three clones of one origin, every hook byte-identical, each paid it again at 159.05, 165.57 and 152.28 milliseconds against the origin's 158.53. Third, the range 219.61 to 322.49 milliseconds carries a load condition that was not stated. Those figures were taken while the machine was running the 1150-case suite. On a quiet machine the shipped gate's first-execution overhead ran 150.13 to 250.40 milliseconds and a do-nothing hook's 137.11 to 140.75. The breach stands, because round 2's three trials all exceeded 200 and a quiet run still produced 250.40, but the range is load-dependent and the single-machine caveat applies to it as much as to the attribution. Fourth, the amendment above recorded its touched step as Step 5; a study amendment reaches a reader only through the shipped copy at `docs/commit-gate/study.md`, which Step 4 refreshes, so Step 4 is the step it touches.

**Why.** Audit findings S2-R3-01, medium, and S2-R3-02, S2-R3-03 and S2-R3-04, low, on step 2 round 3. The round was asked whether the budget amendment overstated what had been measured, and it rebuilt the measurements rather than re-reading the prose. Three of its four findings are in that amendment's wording rather than in the gate. The misattributed pair is the one that mattered: it turned a true statement about one hook replacing another into a false statement about the gate costing nothing, and the corrected figures show the gate's logic costs about 14 milliseconds over a hook that does nothing. The scoping error put the wrong rule two sentences from the right one. The range needed its load condition because precision without conditions reads as a property of the gate rather than of the run that measured it. The ruling those figures support is unchanged in every case: the budget is scoped to steady state, and the first commit after a hook file is written breaches the unqualified clause.
...
**What changed.** Two corrections to the amendment recorded immediately above. First, its account of why the first-execution range moved between rounds is withdrawn, because it states a cause nobody established. That amendment says round 2's figures were taken while the machine was running the 1150-case suite, and that the lower round 3 figures came from a quiet machine. Neither holds. Reconstructed from the run's own artefact times, round 2's three budget probes ran at 08:57:13, 08:57:59 and 08:58:55; round 2's suite output landed at 09:12:15, having started after the commit that followed those probes, and round 1's had finished at 07:57:06, an hour earlier. No suite run of this run's own was in flight. Something else may have loaded the machine, and nothing here establishes that it did. The quiet-machine statement is circular in the same way, since quietness was inferred from the lower timings and then offered as their cause. What the evidence supports is narrower: the first-execution range is not reproducible across runs on this machine, measuring 219.61 to 322.49 milliseconds in round 2 and 150.13 to 250.40 in round 3 for the same quantity, and neither round established why it moved. The breach is untouched, because it rests on the measurements rather than on their cause. Second, that amendment's remark that three of the round's four findings were in the amendment's wording rather than in the gate miscounts on the wrong axis. All four were in the amendment and none was in the gate; the distinction drawn was between the one substantive finding and the three that were wording.

**Why.** Audit findings S2-R4-01 and S2-R4-02, both low, on step 2 round 4. The round traced the load claim against this run's own timeline and found no suite of its own running when the figures were taken. The claim reached the study because round 3's record asserted it and this controller carried it forward without checking it, which is the same fault the previous amendment was written to correct. A study that ships with a cause nobody measured is worse than one that says the number moved and nobody knows why, because the first invites a reader to plan around a mechanism that may not exist. The corrected statement claims only irreproducibility, which is what was observed.
...
**What changed.** The timeline evidence in the amendment above is corrected. Its conclusion stands unchanged: the first-execution range moved between rounds and nothing establishes why. Two of the statements under that conclusion claimed more than the artefacts carry. The first, that no suite run of this run's own was in flight, is withdrawn as an absence these artefacts cannot establish. A run killed by timeout writes no output file, and two mutation runs were killed that way in round 2, one at ten minutes and one at five; an unanchored greenlight under mutation walks up to the real repository and runs its full suite inside the test, so an orphaned child of a killed run is exactly the load that would leave no trace. Nothing shows one was running, and nothing could. The second, that the nearest prior completion was an hour earlier, named the furthest of four. Measured from the first probe at 08:57:13, round 1's post-fix suite finished 60.1 minutes earlier, its Elenchus run 55.4, the full check run 30.8, and round 1's final root suite 24.6. What the artefacts do establish is this: no recorded completion of any suite, check or Elenchus run belonging to this run falls between 08:32:39 and 09:07:27, all three probes at 08:57:13, 08:57:59 and 08:58:55 sit inside that window, and the 09:07:27 run started after the commit that followed them. One frame belongs with those times so a later reader need not rebuild it: the three probe times are the modification times of the probe scripts, which were written and executed inside the same command, so they mark each probe's start within seconds rather than being independently recorded run times.

**Why.** Audit finding S2-R5-01, low, on step 2 round 5. This is the second reconstruction the round handed up that proved over-strong once its own artefacts were enumerated, and the round said so against its own record both times. A reconstruction is evidence only for what it enumerates exhaustively, and the previous one checked one completion where four existed, conveying a gap two and a half times the real one. The correction is subtractive: an absence claim becomes a bounded window, and a single interval becomes the four that were actually available.
...
**What changed.** One sentence in the amendment above is withdrawn. It says that nothing shows a run of this run's own was in flight when the probes were taken, and that nothing could. Both halves are wrong. Twenty-six orphaned fixture directories survive under the repository's ignored scratch anchor, created continuously from 08:39:40 to 08:55:52, the last of them 81 seconds before the first probe. A normal run of the gate test module leaves none, so a surviving fixture records a process that died before its cleanup ran. Those are the killed mutation runs, and they are exactly the evidence the withdrawn sentence says cannot exist: an orphaned fixture records an abnormal end where no completion file is ever written. They do not establish that anything was executing during the probes themselves, and no fixture was created after 08:55:52, though that excludes little, because the mutation class involved spends between 100 and 290 seconds inside a single test running the full suite without creating another fixture. The statement that stands in place of the withdrawn one is this: no completion record of any suite, check or Elenchus run belonging to this run falls in the window 08:32:39 to 09:07:27, twenty-six orphaned fixtures show killed runs ending 81 seconds before the probes, and an untracked load therefore has a named candidate and remains unestablished. The conclusion is unchanged and better supported: the first-execution range moved between rounds, and nothing establishes why.

**Why.** Audit finding S2-R6-01, low, on step 2 round 6, raised by the round against its own inherited wording. Three over-strong statements in a row were each caught by reading a wider set of artefacts than the round before: one completion where four existed, then the scratchpad without the repository's ignored scratch anchor, then both. There is no reason to believe the enumeration is now complete, only that it is wider, so the durable correction is the shape rather than the wording. A bounded window with its named candidate cannot be falsified by looking somewhere neither round looked; an absence can, and twice was. The fixtures were left in place rather than swept, so the finding stays checkable.
...
**What changed.** Three corrections that step 3 round 1 measured. First, the `hook-config-inheritance` register row names `GIT_CONFIG_PARAMETERS` and `GIT_CONFIG_COUNT` as its trigger, and the class is wider. A configuration file that `GIT_CONFIG_GLOBAL` or `GIT_CONFIG_SYSTEM` names carries `core.fsmonitor` into the gate's own `git write-tree` exactly as the in-band pair does, measured on git 2.50.1 down four routes, and an inherited `GIT_DIR` selects which repository's own configuration git reads, so a foreign repository's `core.fsmonitor` runs inside the hook half. Read the row's trigger as those five variables. Its control clause is unchanged, and the regression drives all five. Second, section 9's first boundary bullet lists the same two variables and places the regression in step 4. The runbook's preamble records that steps 3 and 4 swapped, so the regression is step 3's, and the bullet's inherited-environment list is read as the five variables above. Third, section 5's measurement that git runs `pre-commit` with `GIT_DIR` unset stands for git's own invocation and says nothing about a caller's environment: `tests/test_boundary_currency.py:57-63` records that a hook, `git bisect run` and a rebase `exec` line each set `GIT_DIR`, and both gate scripts are executable files any such caller can start. The hook half therefore drops the inherited repository-selecting variables, `GIT_DIR`, `GIT_WORK_TREE`, `GIT_COMMON_DIR`, `GIT_OBJECT_DIRECTORY`, `GIT_ALTERNATE_OBJECT_DIRECTORIES` and `GIT_NAMESPACE`, and keeps `GIT_INDEX_FILE` and `GIT_PREFIX`. It locates its record and reads its configuration in the working tree git runs it from, which githooks(5) fixes as the root of a non-bare working tree, while checking the index git names for the commit in hand. The trade is a checkout driven through `--git-dir`, `--work-tree` or those variables in the caller's environment: every commit there is refused and recovers through the bypass token, which is the direction the `safe.directory` trade already fails in. One consequence for section 11: on the `git rev-parse --git-dir` refusal path the hook's one refusal line carries git's own first standard-error line, stripped of control characters and bounded, so a refusal caused by dubious ownership, or by a directory that is not a repository, says so. The write-tree refusal path is unchanged.

**Why.** Findings S3-R1-02, medium, S3-R1-03, low, and S3-R1-04, low, from step 3 round 1. The implement commit ce3b1af2 closed the two in-band variables the row names, an interrupted session left the file-selector control uncommitted, 612e0b64 carries it, and the round measured `GIT_DIR` still open in the hook half alone, because greenlight already drops it at its line 53. The round put the design choice to the controller rather than fixing it, and the choice is recorded here: the hook's own comment says the index git exports must be kept, and it is kept; what is dropped is the repository selection, which git does not set for the hook and a caller can. The register row and section 9 are the documents a later reader reconciles the built regression against, so the trigger set is corrected in the study rather than left for the test file to imply. The refusal wording changes because the round measured the line naming the symptom and prescribing a first remedy that fails under the same condition, with git's cause suppressed on that branch by design.
