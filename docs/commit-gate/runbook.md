...
**What changed.** Complete replacement Exit: `docs/commit-gate/demonstration.md` records one run of the demo path against a clone of the run branch made into a fresh directory: the root suite refused before activation and the transcript shows the activation command in its output; the activation command was run once; `.githooks/greenlight` recorded a green; a commit of that tree succeeded; a file was then edited and the next commit was refused; and `FIAT_SKIP_PRECOMMIT=1` let that same commit through. Each of the five acceptance conditions is answered with the exact command and its exit code, and anything the run could not establish is named. Proved by `python3 -m unittest tests.test_commit_gate -v` at exit zero and `python3 scripts/run_checks.py --full` at exit zero on the committed tree. Additionally, `.githooks/README.md` no longer says the visibility assertion is still to come: its paragraph on an unactivated checkout describes what the shipped assertion does, rather than promising it with a future step, and the demonstration record's second acceptance condition is the observed run that shows it. Its paragraph on the bypass token no longer says that token is the only way past the gate, because `git commit --no-verify` is the other way and leaves no trace that the gate was skipped; the paragraph says what the token is for and names the other route, so a contributor grepping for the literal is not told it is the only one. Complete replacement Files: `docs/commit-gate/demonstration.md`, `docs/commit-gate/runbook.md`, `.githooks/README.md`, `.horos/boundary.json`.

**Why.** Audit finding S5-R3-02, low, from step 5 round 3. `.githooks/README.md:32` says "That token is the only way past the gate." The round measured otherwise in a scratch fixture carrying the tracked hook with `core.hooksPath` set and a green recorded for tree c837c2e2f571c0749e6ae0cf5d1608cfac38e927: a plain commit of moved tree 74bee5dc0345e80916b92c045b2242d3a4366979 is refused at exit 1 naming both trees, and `git commit --no-verify` of that same tree lands at exit 0 as dfd668b, silently. The study says as much itself at `docs/commit-gate/study.md:381-382`, glossing the bypass as letting a commit through without touching `--no-verify`, so the shipped README contradicts the design it documents. The round left it open because the previous Exit addition governed one paragraph and this is a different one, which is the same scope ground three earlier rounds gave for a stale paragraph elsewhere in the file. That ground is right about scope and wrong about consequence here: a stale framing costs a reader nothing, and a false claim about the one control a contributor is told to grep for costs them the thing the sentence exists to give. The file is already in this step's Files, so the correction needs no new path, only a clause that governs the paragraph. The baseline Exit is restated unchanged above so the replacement is complete rather than a fragment.
