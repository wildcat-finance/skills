...
Accepted, 2026-08-30; renumbered 2026-08-31 and again 2026-09-02. This record
fixes the framework rule selected for issue
...
Step 4 amendments moved the standing identifier to ADR-066. The default branch
then assigned ADR-066 through ADR-068, with its own ADR-066 publishing the
skills.sh payload from a separate repository, so the base sync moves this
unchanged decision to ADR-069. Earlier identifiers remain historical bytes in
the receipted study and runbook prefixes.
