# ADR-101: Build the outer checkpoint archive natively
...
generation. ADR-028 stays Accepted and its inherited bytes stay unchanged.
This record carries the native-archive amendment prepared on 2026-09-07.
The checkpoint store path, the two boundaries, unconditional saving, the
mandatory local hand-off and the rule that no checkpoint operation uploads,
posts, commits or pushes all stand.
