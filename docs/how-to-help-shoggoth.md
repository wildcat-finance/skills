...
The probed roster below is generated from `docs/harness-classification.json`,
which a probe writes by looking at the host it runs on. Where a row has a route
at all, it is a manual one: read `AGENTS.md`, then send the exact `job.prompt`.
A row marked `unsupported` has no route to give, and its blocker says why.
