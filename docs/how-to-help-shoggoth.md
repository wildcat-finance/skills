...
That roster is generated from `docs/harness-classification.json`, which a probe
writes by looking at the host it runs on. Where a row has a route at all, it is
a manual one: read `AGENTS.md`, then send the exact `job.prompt`. A row marked
`unsupported` has no route to give, and its blocker says why.

<!-- harness-roster:begin -->
<!-- Generated by scripts/render_harness_roster.py from docs/harness-classification.json. Change the roster in scripts/probe_harnesses.py, re-run the probe, then re-run the renderer. Nothing between these markers is edited by hand, and the guide names no harness from the probed roster outside them. Codex and Claude Code are named outside them on purpose: they are not in the roster and no probe reads them. -->
...
The canonical manifest keeps the observation host, date and base ref. A row cannot reach `Atlas launcher` or `tested local route` without a client run somebody got an answer from, so every row below carries the exact reason it stopped where it did:
...
Use any repository-valid signing identity. Fiat admits a commit by its valid
signature, not its author, committer, co-authors, trailers, runtime, or byline.
Neither Shoggoth provenance trailer is mandatory. Authenticated local GitHub
access and an authenticated connector have equal standing when they supply the
same exact fields for the bounded check. Publication authority stays separate.
...
That setting is an optional presentation preference. Fiat still reads the
pull-request body back as exact evidence, but host attribution is permitted.
...
Fiat records whether pushed attribution reached the base. For every commit
...
Atlas as the tie-break. It excludes known non-human accounts, the Shoggoth account, and
