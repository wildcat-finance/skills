...
class EvolutionContractTests(unittest.TestCase):
    ...
    def test_fiat_state_shape_frontier_holds_the_task_identity_successor(self):
        ...
        self.assertIn("skills#923", latest["evidence"])
        self.assertIn("fiat-step-branch-extensions-study.md", latest["evidence"])
        self.assertIn("fiat-step-branch-extensions-runbook.md", latest["evidence"])
        self.assertIn("ADR-021", latest["evidence"])
        ...
        authorship_bound = history_rows(ledger)[-4]
        ...
        config_guard = history_rows(ledger)[-8]
