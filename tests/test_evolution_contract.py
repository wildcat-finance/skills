...
class EvolutionContractTests(unittest.TestCase):
    ...
    def test_fiat_state_shape_frontier_holds_the_task_identity_successor(self):
        ...
        self.assertEqual(predecessor["version"], "fiat-v5.38.1")
        self.assertIn("skills#906", predecessor["evidence"])
        self.assertIn("skills#903", predecessor["evidence"])
        self.assertIn("ADR-052", predecessor["evidence"])
        bound = history_rows(ledger)[-3]
        self.assertEqual(bound["version"], "fiat-v5.37.1")
        self.assertIn("skills#556", bound["evidence"])
        self.assertIn("ADR-006", bound["evidence"])
        self.assertIn("fiat-version-relations", bound["evidence"])
        ...
        config_guard = history_rows(ledger)[-6]
