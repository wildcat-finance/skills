...
class EvolutionContractTests(unittest.TestCase):
    ...
    def test_fiat_task_identity_frontier_holds_the_audit_history_successor(self):
        ...
        self.assertEqual(field(ledger, "Current version"), "fiat-v6.76.1")
        self.assertEqual(field(ledger, "Frontier status"), "open")
        self.assertEqual(field(ledger, "Frontier revision"), "delegated-task-identity")
        self.assertEqual(field(ledger, "Current frontier"), FIAT_FRONTIER)
        self.assertEqual(field(ledger, "Next Fiat job"), FIAT_NEXT_JOB)
        current = history_rows(ledger)[-1]
        self.assertEqual(current["version"], "fiat-v6.76.1")
        ...
        self.assertIn("test_checkpoint_suite_prerequisites.py", current["evidence"])
        self.assertIn("skills#1927", current["evidence"])
        self.assertIn("never the caller's `PATH`", current["change"])
        framework = next(row for row in history_rows(ledger) if row["version"] == "fiat-v6.75.1")
        self.assertIn("test_issue_filing_contract.py", framework["evidence"])
        self.assertIn("skills#1558", framework["evidence"])
        self.assertIn("older em-dash form", framework["change"])
        adapter = next(row for row in history_rows(ledger) if row["version"] == "fiat-v6.74.1")
        self.assertIn("test_gate_commands.py", adapter["evidence"])
        self.assertIn("every other gate field agrees", adapter["change"])
    ...
    def test_elenchus_parent_guard_generation_stays_inside_its_mature_frontier(self):
        ...
        self.assertEqual(field(ledger, "Current version"), "elenchus-v1.11.0")
        self.assertEqual(field(ledger, "Frontier status"), "mature")
        self.assertEqual(
