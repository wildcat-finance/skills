...
class EvolutionContractTests(unittest.TestCase):
    ...
    def test_fiat_state_shape_frontier_holds_the_task_identity_successor(self):
        ...
        self.assertEqual(field(ledger, "Current version"), "fiat-v5.34.1")
        self.assertEqual(field(ledger, "Frontier status"), "open")
        ...
        latest = history_rows(ledger)[-1]
        self.assertEqual(latest["version"], "fiat-v5.34.1")
        self.assertEqual(latest["axis"], "generation")
        ...
        self.assertIn("Creator direction, 2026-08-29", latest["evidence"])
        self.assertIn("audit.max_rounds", latest["evidence"])
        predecessor = history_rows(ledger)[-2]
        self.assertEqual(predecessor["version"], "fiat-v5.33.1")
        self.assertIn("Creator direction, 2026-08-28", predecessor["evidence"])
        self.assertIn("completed run", predecessor["evidence"])
        earlier = history_rows(ledger)[-3]
        self.assertEqual(earlier["version"], "fiat-v5.32.1")
        self.assertIn("skills/issues/710", earlier["evidence"])
        self.assertIn("issuecomment-5451995033", earlier["evidence"])
        integrated = history_rows(ledger)[-4]
        self.assertEqual(integrated["version"], "fiat-v5.31.1")
        self.assertIn("skills/issues/710", integrated["evidence"])
        self.assertIn("ADR-044", integrated["evidence"])
        base_fix = history_rows(ledger)[-5]
        self.assertEqual(base_fix["version"], "fiat-v5.30.1")
        self.assertIn("skills/issues/622", base_fix["evidence"])
        self.assertIn("Creator direction, 2026-08-27", base_fix["evidence"])
        published = history_rows(ledger)[-6]
        self.assertEqual(published["version"], "fiat-v5.29.1")
        self.assertIn("issuecomment-5435028801", published["evidence"])
        self.assertIn("issuecomment-5435304048", published["evidence"])
        base_head = history_rows(ledger)[-7]
