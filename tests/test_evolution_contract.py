...
class EvolutionContractTests(unittest.TestCase):
    ...
    def test_fiat_state_shape_frontier_holds_the_task_identity_successor(self):
        ...
        amendment_relation = history_rows(ledger)[-2]
        self.assertEqual(amendment_relation["version"], "fiat-v5.42.1")
        self.assertEqual(amendment_relation["axis"], "generation")
        self.assertEqual(
            amendment_relation["revision"], "state-shape-validation"
        )
        self.assertIn("skills#497", amendment_relation["evidence"])
        self.assertIn(
            "next-generation-after-integration-base", amendment_relation["change"]
        )
        step_extensions = history_rows(ledger)[-3]
        ...
        sync_guard = history_rows(ledger)[-4]
        ...
        complete_graph = history_rows(ledger)[-5]
        ...
        authorship_bound = history_rows(ledger)[-6]
        ...
        relation_bound = history_rows(ledger)[-7]
        ...
        path_bound = history_rows(ledger)[-8]
        ...
        config_guard = history_rows(ledger)[-10]
        ...
        earlier = history_rows(ledger)[-11]
        ...
        integrated = history_rows(ledger)[-12]
        ...
        base_fix = history_rows(ledger)[-13]
        ...
        published = history_rows(ledger)[-14]
        ...
        checkpoint = history_rows(ledger)[-15]
        ...
        base_head = history_rows(ledger)[-16]
