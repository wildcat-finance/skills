<!-- front-door-specimen: expect="FD20" reason="a card claims real data for a record the tree records as mixed" -->
<p align="center">
...
### QUARRY REBUILDS ITS HELD SPECIMEN

<!-- front-door:demo skill="quarry" claim="{{claim:quarry}}" digest="{{digest:quarry}}" -->
Quarry rebuilds the specimen it preserved.

Run `python3 scripts/demonstrations.py run --record {{directory:quarry}} --report tmp/demo/quarry.json`
over the preserved `{{source:quarry}}` and it reports `{{observed:quarry}}`.
{{nonclaim:quarry}}
