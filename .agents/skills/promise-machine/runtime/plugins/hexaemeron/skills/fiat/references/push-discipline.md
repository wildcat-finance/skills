...
parents. Its final trailer block
contains exactly one `ADR-Assignment-Base: <base>` followed by the report's
ordered `ADR-Assignment: adr/<slug>=ADR-NNN` lines. Verify the immutable result
before publication:
