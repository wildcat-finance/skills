...
def done_sync_run(args, state: dict) -> None:
    ...
    if remote_branch_tip(
        ...
        die("run sync commit moved during evidence collection")
    _require_native_relation_history(args.dir)
    if _native_relation_repository_identity(args.dir) != repository:
    ...
    if sync:
        ...
        if assignment_receipt is not None:
            ancestry = _native_ancestry_status(
                args.dir, sync.get("commit"), args.merge_commit
            )
            if ancestry is None:
                die("integration assignment ancestry cannot be determined")
            if ancestry != 0:
                die("active decision assignment is not in integration ancestry")
...
def verify_run(
    ...
    *,
    ...
    terminal_sync = terminal.get("sync")
    if terminal_sync is not None and terminal_sync != active_sync:
        die("terminal integration sync does not copy the active receipt", 1)
