...
stops at cohort construction and refuses every operation past it. Hexaemeron contains Fiat, Kronos, six phase
...
| Build one checked cross-run cohort, or inspect the comparison boundary and its held operations | [Synkrisis](../../../plugins/synkrisis/AGENTS.md) | `synkrisis` |
...
Synkrisis is outside Fiat's controller loop. Its delivered surface builds one
checked cohort and stops there: it cannot emit a finding, file an issue,
mutate a repository, or dispatch a Fiat run. A future comparison result will
still require a person to choose any next owner and action.
