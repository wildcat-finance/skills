...
**What changed.** Complete replacement Files: `plugins/dokimasia/scripts/dokimasia_lib/reconcile.py`; `plugins/dokimasia/scripts/dokimasia.py`; `plugins/dokimasia/schemas/coverage-v1.json`; `plugins/dokimasia/schemas/dispositions-v1.json`; `plugins/dokimasia/tests/fixtures/dispositions/`; `plugins/dokimasia/tests/test_reconcile.py`; `plugins/dokimasia/tests/test_scaffold.py`; `plugins/dokimasia/docs/coverage-contract.md`; `plugins/dokimasia/docs/evidence/wildcat-app-v2.coverage.json`; `plugins/dokimasia/docs/evidence/wildcat-app-v2.scrutiny.json`; `plugins/dokimasia/docs/evidence/wildcat-app-v2-scrutiny.md`.
Complete replacement Tests: Cover an all-unconfirmed set closing at zero and naming every entry; a mixed set closing at exactly the confirmed count; an entry missing the `confirmed` field refusing by name; a non-boolean confirmation refusing by name; an unconfirmed `covered` entry, which must refuse for the reasons `covered` already refuses rather than being excused by being unconfirmed; one item answered twice across the confirmed and unconfirmed lists; the three counts agreeing with the scoped set three ways as the existing arithmetic tests require; and the emitted record validating against the committed schema. Also cover the report identity: that the default candidate and criterion are unchanged, that a named candidate and criterion reach the emitted report, and that every verb writing a report accepts both flags. Extend the committed fixtures rather than replacing them, so the prior run's cases keep running. Every new cap is a parameter, never a module-level value a caller can lower, which is the pattern S2-R1-01 and S3-R1-03 both recorded. Elenchus runner: `python3 plugins/dokimasia/scripts/dokimasia.py reconcile --check --candidate confirmed-flag --criterion proposal-refused-unconfirmed --report {report}`; report format `protasis-design-report/v1`; report file `.hexaemeron/reports/conformance/confirmed-flag-proposal-refused-unconfirmed.json`.

**Why.** The step's own exit command produced a report the design checker refused at `step:3`, because `dokimasia.py` pinned the reported candidate and criterion to module constants naming the frontier the plugin was first built under. The resolver this run's record names could therefore never satisfy it. Repairing that is a change to `dokimasia.py`, which the receipted Files clause did not name, and it earns the report-identity tests the replacement Tests clause adds. The evidence records join Files because the step regenerates the pinned scrutiny to carry the two new fields.
...
**What changed.** Complete replacement Exit: A proposal is generated for the pinned pair, a subset is confirmed by hand and committed as evidence, and the scrutiny reports a closure ratio above zero whose numerator counts confirmed entries only; the committed scrutiny and coverage records carry the confirmed and unconfirmed counts and validate against their schemas; a second scrutiny of the same inputs agrees; the ledger carries exactly one new row at `dokimasia-v2.1.0`, incrementing evolution and retaining generation and epoch, with the frontier line, its SHA-256 and one evidenced next job; the SKILL.md marketplace block, the README frontier prose, both host manifests, the root marketplace entry and the pinned delivery version all name the same version; the new promise carries a coverage row, its evidence entries and a runtime binding, and the binding whose result surface moved is refreshed; the Horos boundary is regenerated after every file has landed, with its count read back rather than piped away, which is S5-R5-01's correction. Proved by `python3 scripts/run_checks.py --base fiat/dokimasia-proposed-dispositions && python3 plugins/dokimasia/scripts/dokimasia.py demonstrate --check --report .hexaemeron/reports/conformance/confirmed-flag-pinned-closure-above-zero.json`.
Complete replacement Files: `plugins/dokimasia/docs/evidence/wildcat-app-v2.dispositions.json`; `plugins/dokimasia/docs/evidence/wildcat-app-v2-scrutiny.md`; `plugins/dokimasia/docs/evidence/wildcat-app-v2.coverage.json`; `plugins/dokimasia/docs/evidence/wildcat-app-v2.scrutiny.json`; `plugins/dokimasia/skills/dokimasia/EVOLUTION.md`; `plugins/dokimasia/skills/dokimasia/SKILL.md`; `plugins/dokimasia/README.md`; `plugins/dokimasia/.claude-plugin/plugin.json`; `plugins/dokimasia/.codex-plugin/plugin.json`; `plugins/dokimasia/scripts/dokimasia.py`; `plugins/dokimasia/tests/test_scaffold.py`; `.claude-plugin/marketplace.json`; `.horos/boundary.json`; `.horos/candidates.json`; `tests/promise_machine_coverage.json`; `tests/test_promise_machine_contract.py`; `tests/test_version_propagation.py`.

**Why.** The receipted Exit named `run_checks --full`, which cannot pass from this branch through no fault of this run: `tabularium-suite` fails on `main` at the exact commit this run was cut from, because `plugins/tabularium/skills/tabularium/EVOLUTION.md` links `../../../../VENUES.json` outside its own plugin root. That is the ownership question pull request #1094 asked a reviewer to settle, verified failing at `f3af97e9` before this amendment was written. The scope-selected form runs every check this run's changed paths own, which is nine checks including the root suite, and omits the suites this run does not touch. The Files clause also grew: moving the version and registering a new promise reaches the repository's own coverage, version and boundary records, none of which the receipted clause named.
