...
- For a canonical Fiat audit ledger, first require `audit_synopsis.py --check .`
  on the same tree, then use explicit mode `fiat-audit-record`; it suppresses
  only the generic B010 and B011 cardinalities and establishes no Fiat schema.
