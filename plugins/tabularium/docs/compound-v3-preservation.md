...
Compound III needs a new canonical event schema version. The current Aave v4
consensus-log and Euler releases carry schema version 2, which cannot be
widened in place.
Phase 1 therefore adds schema v3, or the next available version if another
release lands first.
