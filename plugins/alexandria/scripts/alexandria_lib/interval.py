...
def validate_checkpoint(checkpoint, plan_digest: str, shard_count: int) -> None:
    ...
    if checkpoint["plan_sha256"] != plan_digest:
...
class Staging:
    ...
    def record(self, shard: int, name: str, request: bytes, response: bytes) -> None:
        ...
        self._handle(name).write(canonical_bytes(entry))
        self._records += 1
