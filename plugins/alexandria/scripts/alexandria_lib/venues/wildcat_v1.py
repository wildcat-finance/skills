"""The Wildcat V1 venue: established source identity, an unresolved checkout and declared gaps.

`validate_registry` is the unedited `validate_v1_registry` function
`wildcat_registry.py` defines, re-exported rather than wrapped, so the
registry's pinned digest is written in one module only.

The epoch model is `immutable-code`, the same model Wildcat V2 uses: no
Wildcat V1 subject is an upgradeable proxy, so each declared subject has
exactly one epoch, opening at the later of the interval start and the
subject's own deployment block when the registry carries one, and otherwise
at the first in-interval block this collection observes runtime code at. The
mechanics duplicate `wildcat_v2`'s rather than importing them, so a change to
one venue's opening reads never silently reaches the other's pinned document
or its digest.

This module therefore issues no storage read and compares no log topic
against an upgrade announcement: it passes `upgrade_topic=None` to the shared
log walk and never names the EIP-1967 slot or the ERC-1967 topic.

Every one of the 16 declared subjects carries an established source commit,
so this venue's registry declares no coverage gap over source identity; the
mechanism that would declare one for a subject whose row establishes none
stays in the code (`_missing_source_commit_gap`) and fires only against a
constructed row, because the real row has no such subject. What the row does
carry is an unresolved checkout: 14 of the 16 subjects share one recorded
commit among five byte-identical candidates, and the registry records the
recorded commit beside the other four and the row's own explanation, as a
caveat on an established identity rather than as a gap. A release that
asserted one of the four as a subject's own source commit, rather than the
row's recorded one, refuses.

Deployment-block coverage is a real, separate gap: 12 of the 16 subjects
carry no deployment block in their code-match record, so their epoch opens at
whatever block this collection's own opening reads observe, and the release
names the missing deployment block as a gap for each of them.
...
from ..wildcat_registry import subject_entries, validate_v1_registry as validate_registry
...
class ImmutableCodeOpening:
    """The opening reads a Wildcat V1 plan owes, in the order they are made.
...
def _missing_source_commit_gap(address: str) -> str:
    """The gap mechanism a subject whose row establishes no source commit would carry.

    The real V1 row leaves no subject in this state, so this never appears in
    a real release; it is exercised only against a constructed registry row.
    """
    return (
        f"the merged records establish no source commit for subject {address}, so this "
        "release claims no source identity for it"
    )
...
def gaps(registry, plan=None) -> list[str]:
    ...
    Unlike Wildcat V2's, this venue's registry never withholds a subject's
    own source commit in real data; `_missing_source_commit_gap` stays wired
    in for a constructed row that does, but contributes nothing here.
    """
    entries = registry["entries"]
    result = [
        f"the registry pins no source bytes; every one of its {len(entries)} subjects names a "
        "source commit read from the public wildcat-protocol repository"
    ]
    result.extend(
        _missing_source_commit_gap(entry["address"])
