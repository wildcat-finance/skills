"""The checked-in Aave v4 fixture runs and remains offline reproducible."""
...
class AaveV4DemoTests(unittest.TestCase):
    ...
    def test_fixture_verifies_with_expected_evidence_and_provenance(self):
        ...
        self.assertEqual(
            report["evidence_counts"],
            {"proof_backed": 5, "header_bound": 1, "recorded_rpc": 7},
        )
        self.assertEqual(report["proof_backed"]["accounts_included"], 2)
        self.assertEqual(report["proof_backed"]["storage_included"], 3)
    ...
    def test_demo_command_runs_the_complete_application_check(self):
        ...
        self.assertIn("replayed code bytes: 1419", result.stdout)
        self.assertIn("replayed logs: 1", result.stdout)
        self.assertIn("slot 0x2 miss: -32070", result.stdout)
    ...
    def test_application_replay_and_miss_cannot_leave_loopback(self):
        ...
        self.assertEqual(report["spoke_slot_zero"], "0x" + "00" * 31 + "0b")
...
class AaveV4ReceiptProofDemoTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            command.extend(
                (
                    ...
                    "0x18ac22c",
                    "--block-hash",
                    "0x11e9be2ff9ff6a04319af0b04c24b95f3f1117c2df79f44f94d208857d01af07",
                )
            )
            result = subprocess.run(
                command,
    ...
    def test_builder_command_materializes_the_byte_identical_fixture(self):
        with tempfile.TemporaryDirectory() as directory:
            rebuilt = Path(directory) / "tmp" / "aave-v4-spoke-v1"
            ...
            self.assertEqual(event["event"], "aave_v4_fixture_build")
    ...
    def test_every_fixture_mutation_materializes_before_verification(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            after_log = load(changed / "receipt-witness.json")["receipts"][0x3F][
                "logs"
            ][0]
    ...
    def test_demo_command_emits_one_bounded_structured_event(self):
        ...
        self.assertEqual(event["correlation_id"], "aave-v4-spoke-v1-offline-demo")
