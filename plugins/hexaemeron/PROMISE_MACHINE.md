...
- Consequence: 1
- Refuses: An account type other than User or Bot, a Bot absent from the ranking's local non-human set, a merged pull request without a classifiable author, a login failing the GitHub login grammar, a repository argument carrying query syntax, a duplicate repository source, any failed API read including a rate limit, an excluded login reaching the ranked output, a `README.md` that is absent or not UTF-8, and a read that would silently truncate.
- Recovery: Read the stop, which names the identity or field at fault; extend the local non-human set in `scripts/contributors.py` for an unknown non-human account, set a token or wait for the named reset for a rate limit, and rerun with `--write` for a stale artefact. The generator never repairs an input.
- Exceptions: none
<!-- local contributor classification only -->
