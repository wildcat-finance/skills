...
def install_fake_gh(repo: Path) -> dict:
    """Answer the repository and task-issue reads init makes, without a network.

    The body carries the filing contract every fixture issue satisfies, so a
    case that is not about that contract does not have to restate it.
    """
    fake_bin = repo / "delivery-tools"
...
if re.fullmatch(r"repos/[^/]+/[^/]+", path):
    print(json.dumps({"full_name": path.split("/", 1)[1]}))
    raise SystemExit(0)
issue = re.fullmatch(r"repos/[^/]+/[^/]+/issues/(?P<number>[0-9]+)", path)
if issue:
    print(json.dumps({"number": int(issue.group("number")), "body": DEFAULT_ISSUE_BODY}))
    raise SystemExit(0)
raise SystemExit(1)
""",
        encoding="utf-8",
...
class HexctlCheckpointIdentityTests(unittest.TestCase):
    def setUp(self):
        ...
        self.env = install_fake_gh(self.repo)
    ...
    def test_init_named_base_is_resolved_once_before_worktree_add(self):
        ...
        current = {
            # Read from the controller rather than pinned here: init refuses a
            # currency reading whose version differs from the one it resolved,
            # so a literal would fail the next time Fiat's ledger moves.
            "ledger_version": module.init_controller_identity()["version"],
...
class HexctlSemanticCheckpointIdentityTests(HexctlCase):
    ...
    def test_identity_with_observation_writes_no_validator_bytecode(self):
        ...
        with tempfile.TemporaryDirectory() as runtime_directory:
            ...
            # Controller verification runs the Protasis design-evidence checker,
            # and the controller finds it relative to its own file. Repointing
            # __file__ therefore has to bring the checker along, or the run
            # refuses for a missing checker rather than telling us anything
            # about where bytecode was written.
            fake_checker = (
                runtime
                / "plugins"
                / "hexaemeron"
            ...
            shutil.copyfile(
                HERE.parent / "skills" / "protasis" / "scripts" / "design_evidence.py",
                fake_checker,
            )
