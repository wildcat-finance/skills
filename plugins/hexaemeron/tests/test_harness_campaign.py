...
or one left behind after the suite changed, disagrees with the fetched
...
PROVENANCE = os.path.join(HARNESS, "PROVENANCE.json")
...
def pairing():
    """The pairing suite's emitter count, derived from the fetched emitter files."""
...
class HarnessCampaignTests(unittest.TestCase):
    ...
    def test_the_emitter_count_equals_the_pairing_count(self):
        module = pairing()
        if not module.FETCHED:
            self.skipTest("protocol source not fetched; run plugins/hexaemeron/harness/fetch_protocol.py")
