...
assembly emitters in `wildcat-finance/v2-protocol` at
`f5a26146987926f4811b72a795d662813dedfe85`, fetched by `fetch_protocol.py`
and checked against `PROVENANCE.json`, the log the emitter records
...
- completeness of the event set. `AccountSanctioned`
  ...
  no assembly emitter, so it is out of scope; at the pinned commit nothing under
  the protocol's `src/` emits it, and only `test/helpers/ExpectedStateTracker.sol:288`
  does;
