...
| `sync-run` | When the base advanced and the integration PR conflicts, preserve the completed product evidence and receipt a signed two-parent merge plus bounded integration revalidation; when the composition assigns decision records, replay its canonical report and exact signed mapping; explicitly acknowledge every whole-side or superseded-intersection path the controller names; supersede a failed composition receipt only with the exact active SHA, a reason and fresh evidence | [push-discipline.md](references/push-discipline.md) | `done sync-run --commit <sha> --base-commit <sha> --revalidation .hexaemeron/integration-revalidation.json [--decision-assignments .hexaemeron/decision-assignments.json] [--acknowledge-sync-path <path> ...] [--supersede-sync <sha> --reason <text>]` |
