#!/usr/bin/env python3
"""Run the Horos suite and emit one fresh Elenchus report."""
...
def absolute_report_parts(supplied: Path, root: Path) -> Path:
...
def _private_report(parent_fd: int) -> tuple[str, int, tuple[int, int]]:
    ...
    for _ in range(16):
        name = f".horos-report-{secrets.token_hex(12)}.tmp"
        try:
...
def main(argv: list[str] | None = None) -> int:
    """Run every Horos unit test and preserve its suite exit."""
