...
| MetaMorpho vaults | unimplemented | none | -- | 0 | 450 vaults on mainnet, on the same keyless API. A counterparty may appear here as a curator rather than a borrower, and a curator who allocated into a market that took bad debt made a call that cost lenders money. Not collected yet. |
| Morpho Blue | checked | fixtures | fixture | 3 | ethereum mainnet only; 3 record(s) across 1 address(es) |
| Morpho Midnight | unimplemented | none | -- | 0 | Fixed-rate, fixed-maturity lending on a separate keyless REST API at api.morpho.org/v0/midnight, not the GraphQL one. A maturity means there is a date by which the money was due, so this reads closer to Wildcat than Blue does. Base rather than mainnet. Not collected yet. |
